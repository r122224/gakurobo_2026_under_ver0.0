...
//   con.update();
  ...
  while (true) {
      ...
      //timer.reset();
        ...
        if(!flag_simu){
            ...
            if(set_height_front){
                ...
                front_lift_posi = -(-0.0683 * get_lift_front_posi)*10*2;
            }
            if(set_height_back){
                double back_lift_diff = enc_to_lift_height(get_lift_back_count,pre_get_lift_back_count,FRONT_R);
                get_lift_back_posi += back_lift_diff;
                // back_lift_posi = (-0.0677 * get_lift_back_posi - 0.0626)*10*2;
                back_lift_posi = -(-0.067 * get_lift_back_posi)*10*2;
            }
            if(limit8read == false ){
            ...
            if(lrtb_comm && lrtb.update()){
                ...
                if(mode == MODE_ZONE1){//ゾーン1，phase 事に変更
                    ...
                    if(flag_lrtb_back){
                        if(autonomous.phase == 200 || autonomous.phase == 210 || autonomous.phase == 3 || autonomous.phase == 301 || autonomous.phase == 302 || autonomous.phase == 303 || autonomous.phase == 304 || autonomous.phase == 315 || autonomous.phase == 306 || autonomous.phase == 325  || autonomous.phase == 335 || autonomous.phase == 4){//ラック衝突
                            lrtbPosi.x = distance_back;
            ...
            //limitスイッチ自己位置補正
            if(autonomous.phase == 301 && !limit2read && !limit3read){
            ...
            //limitスイッチによる自己位置補正（誤差の修正なのでgPosi.zを基準に．．．）
            if((autonomous.phase == 11) && !limit4read && !limit5read){//後
    ...
    if(flag_print && set_print){
        ...
        invoke_print(str);
        // // // //速度指令
        sprintf(str,"%4.4lf,%4.4lf,%4.4lf ", refV.x, refV.y, refV.z);
        invoke_print(str);
